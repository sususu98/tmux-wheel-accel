...
- 6 档自适应变速箱：从 1档逐行精读（1 行）、2档触控板温和巡航（1 行），到 6档无极飞轮红线疾速起飞（16 行/次），无缝自适应升档。
- 亚毫秒级同帧子包去抖（debounce_min_dt_ms）：自动过滤 macOS/iTerm2 在单个 60Hz 帧内拆分发出的 `< 3.5ms` 成对子包，彻底消灭触控板偶发暴冲。
- 触控板连击深度熔断保护（high_gear_min_streak）：触控板即使快速轻拂，也会被严格锁在 1~3档（1~2 行），绝不会发生“划一下飞掉 800 行”的失控现象。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 档 | 单格慢拨 / 逐行精读 | $\ge 45\text{ms}$ | 任意 | 1 行 | 精准核对单行代码、无误跳 |
| 2 档 | 触摸板手势 / 慢速连续巡航 | $12\text{ms} \sim 45\text{ms}$ | 任意 | 2 行 | 触控板主力档，平稳流畅不拖沓 |
| 3 档 | 较快翻阅代码 / 触控板快划 | $7\text{ms} \sim 12\text{ms}$ | $\ge 4$ | 3 行 | 快速轻拂翻阅 |
| 4 档 | 快速连续拨轮 | $4.5\text{ms} \sim 7\text{ms}$ | $\ge 4$ | 6 行 | 快速看长函数 |
| 5 档 | G502 无极飞轮中高速 | $3.5\text{ms} \sim 4.5\text{ms}$ | $\ge 25$ (长旋飞轮) | 12 行 | 飞速滑过几百行 |
| 6 档 | G502 物理飞轮红线狂转 | $< 3.5\text{ms}$ (超高频) | $\ge 25$ (长旋飞轮) | 24 行 | 真正 6 档红线起飞，瞬间翻越长日志 |
